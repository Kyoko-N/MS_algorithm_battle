...
def run(inputData):
    ...
    def MergeTrades(trade1,trade2):
        ...
        while trade1 and trade2:
            if trade1[-1][0] < trade2[-1][0]: res.append(trade1.pop())
            else: res.append(trade2.pop())
            if len(res) >= 2 and res[-1][0] == res[-2][0] and res[-1][2] == res[-2][2]: # combine
                if res[-1][1] == res[-2][1]:
                    res[-2][3] += res[-1][3]
                    res.pop()
                else:
    ...
    def CalculateBestStrategyFrom(X,name,start):
        dp = [[0,0] for _ in range(fullLen)] # 不持有的金额 和 持有10000股的剩下的金额
        dp[start-1][1] = -999999999
        log = [{'V':[],'H':[]} for _ in range(fullLen)]
        for i in range(start,fullLen):
    ...
    for n in range(200,500):
        ...
        preA, preB, preC = [], [], []
        for trade in result: # [0, 'buy', 'C', 8]
            if trade[2] == 'A': preA.append(trade)
            if trade[2] == 'B': preB.append(trade)
            if trade[2] == 'C': preC.append(trade)

        postA = CalculateBestStrategyFrom(A, 'A', n)
        postB = CalculateBestStrategyFrom(B, 'B', n)
        postC = CalculateBestStrategyFrom(C, 'C', n)
        ...
        resultMoney = TestTrades(combinedResult)
        if resultMoney > maxMoney:
            maxMoney = resultMoney
